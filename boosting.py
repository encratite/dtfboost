...
def train_lightgbm(x_training: pd.DataFrame, x_validation: pd.DataFrame, y_training: pd.DataFrame, y_validation: pd.DataFrame) -> TrainingResults:
	...
	for num_leaves, min_data_in_leaf, max_depth, num_iterations, learning_rate in tqdm(combinations, desc="Evaluating hyperparameters", colour="green"):
		...
		results.submit_model(model, model_parameters)
	return results

def train_catboost(x_training: pd.DataFrame, x_validation: pd.DataFrame, y_training: pd.DataFrame, y_validation: pd.DataFrame) -> TrainingResults:
	results = TrainingResults(x_validation, y_validation)
...
def train_xgboost(x_training: pd.DataFrame, x_validation: pd.DataFrame, y_training: pd.DataFrame, y_validation: pd.DataFrame) -> TrainingResults:
	results = TrainingResults(x_validation, y_validation)
